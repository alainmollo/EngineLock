...
void Sim800L::SetUp(long baudrate, uint8_t receivePin)
{
	...
	if (_InterruptPin != SIM800_INT_PIN_DEFAULT)
...
bool Sim800L::Init(void)
{
	...
	if (result)
	{
		...
		this->levelSignal = this->signalQuality();
...
// Wait xxx response into an array of response
bool Sim800L::waitResponseArray(String response[], byte count)
{
	Logger.Log(F("Waiting for response:"), false);
	for (int i = 0; i < count; i++)
	{
		Logger.Log(response[i], false);
		Logger.Log(F("/"), false);
	}
	Logger.Log(F(""));
	...
	while (counter++ < RESPONSE_COUNT_OUT)
	{
		String serial = _readSerial();
		for (int i = 0; i < count; i++)
		{
			bool result = serial.indexOf(response[i]) != -1;
			flag |= result;
		}
		delay(10);
		if (flag)
	...
	if (flag && counter < RESPONSE_COUNT_OUT)
	{
		Logger.Log(F("Response is OK"));
...
bool Sim800L::reset(void)
{
	...
	bool flag = waitResponseArray(initOk, sizeof(initOk) / sizeof(String));
